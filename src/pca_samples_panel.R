...
PCA.ui <- function(id, expression.matrix) {
  tabItem(
    ...
    fluidRow(
      box(plotOutput(NS(id, "pca"), height = 500), width = 8, title = "PCA Plot", footer = "The plot represents the scatter of data across chosen principal components. The data-points (genes) are colour-coded by their contribution to the PCA. We note that the individual contributions to a given PC are formally calculated as the
    ratio of the squared factor scores of a given data-point by the eigenvalue associated with that component. Here we have two PCs, in this context the contribution is defined the same way, except as the ratio of the sum of factor scores belonging to the PCs and then sum of eigenvalues."),
...
# Bi-plot from fviz implementation
...
# Variance explained barplot across PCs
